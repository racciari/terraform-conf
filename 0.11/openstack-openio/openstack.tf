...
data "openstack_networking_network_v2" "tf_external_network" {
...
resource "openstack_networking_router_v2" "tf_router1" {
  name                = "router1"
  admin_state_up      = true
  external_network_id = "${data.openstack_networking_network_v2.tf_external_network.id}"
}
...
resource "openstack_compute_instance_v2" "tf_instance" {
  ...
  block_device {
    uuid                  = "${openstack_blockstorage_volume_v3.tf_instance_volume_root.*.id[count.index]}"
    source_type           = "volume"
    boot_index            = 0
  ...
  network {
    port = "${openstack_networking_port_v2.tf_instance.*.id[count.index]}"
  }
...
resource "openstack_compute_volume_attach_v2" "tf_attach_volume_add" {
  count       = "${var.num-nodes * var.num-vol-add}"
  instance_id = "${openstack_compute_instance_v2.tf_instance.*.id[count.index / var.num-nodes]}"
  volume_id   = "${openstack_blockstorage_volume_v3.tf_instance_volume_add.*.id[count.index]}"
}
